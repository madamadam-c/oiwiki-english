...
void addedge(int u, int v) {  // Build graph
  e[tot].v = v;
  e[tot].nxt = head[u];
  head[u] = tot++;
  e[tot].v = u;
...
void dfs_size(int u, int fa) {  // Find size values
  sz[u] = 1;
  maxv[u] = 0;
...
void dfs_center(int rt, int u, int fa, int id) {
  ...
  if (Max == maxv[u]) center[id].push_back(u);  // If equal, push_back
  for (int i = head[u]; i; i = e[i].nxt) {
    int v = e[i].v;
    if (v == fa) continue;
...
int dfs_height(int u, int fa, int depth) {  // Recursively find height
  L[depth].push_back(u);
  f[u] = fa;
  int h = 0;
  for (int i = head[u]; i; i = e[i].nxt) {
...
void init(int n) {  // Initial processing
